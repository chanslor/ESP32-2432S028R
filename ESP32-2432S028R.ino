/*
 * ESP32 River Levels Display for ESP32-2432S028R (CYD)
 * Fetches USGS river data from API and displays on 2.4" ILI9341 TFT
 *
 * Hardware: ESP32-2432S028R (Cheap Yellow Display)
 * Display: 320x240 pixels ILI9341
 *
 * Shows 4 rivers in a 2x2 grid layout
 * - Top row: Locust Fork, Town Creek
 * - Bottom row: South Sauty, Little River
 */
...
// Display dimensions for ESP32-2432S028R
#define SCREEN_WIDTH  320
#define SCREEN_HEIGHT 240

// Layout constants for 2x2 grid
#define HEADER_HEIGHT 30
#define RIVER_BOX_WIDTH (SCREEN_WIDTH / 2)
#define RIVER_BOX_HEIGHT ((SCREEN_HEIGHT - HEADER_HEIGHT) / 2)
...
RiverData rivers[5]; // Locust Fork, Town Creek, South Sauty, Little River, Short Creek
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.fillRect(0, 120, SCREEN_WIDTH, 40, COLOR_BG);
...
void fetchRiverData() {
  ...
  if (httpCode == 200) {
    ...
    for (JsonObject site : sites) {
      ...
      // Map site IDs to our river array
      int index = -1;
...
void displayRivers() {
  ...
  // Draw rivers in vertical list (card deck style)
...
void updateElapsedTimeDisplay() {
  ...
  tft.fillRect(180, 7, SCREEN_WIDTH - 185, 20, COLOR_BG);
...
void drawRiverCard(int riverIndex, int y) {
  ...
  // Line 1: Status dot + River name + Flow + Trend
  int xPos = 10;
  ...
  // Trend (fixed position, stays the same)
  tft.setCursor(xPos + 220, y + 3);
  ...
  if (riverIndex < 4) {
    tft.drawLine(5, y + 35, SCREEN_WIDTH - 5, y + 35, COLOR_BORDER);
  }
}
